...
# APIの仕様上、filtersパラメータは他のパラメータと異なり、"filters=..."という形式ではなく、"filters[viewCounter][0]=1000000"のような形式なので、filtersパラメータが空ではない場合その方法ではデータを取得できないと思う
# （本分析ではfiltersパラメータを与えていないのでhttr::GETでも問題ない）
# そのため、各クエリパラメータを文字列で連結してURLを作る方法を取っている
